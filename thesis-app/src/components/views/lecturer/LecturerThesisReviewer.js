...
const LecturerThesisReviewer = () => {
    ...
    const handleOpenMarking = async (thesisId) => {
        ...
        // Load điểm đã chấm (nếu có)
        try {
            const response = await authApis().get(endpoints.getThesisMark(thesisId));
            if (response.data) {
                // Nếu có điểm đã chấm, điền vào form
                // Nếu điểm là -1 (mặc định) thì hiển thị 0
                setMarkingData({
                    thesisId: thesisId,
                    score: {
                        format: response.data.format === -1 ? 0 : (response.data.format || ''),
                        content: response.data.content === -1 ? 0 : (response.data.content || ''),
                        research: response.data.research === -1 ? 0 : (response.data.research || ''),
                        presentation: response.data.presentation === -1 ? 0 : (response.data.presentation || '')
                    }
                });
            }
        } catch (error) {
            // Nếu chưa có điểm hoặc lỗi, giữ nguyên form trống
                  ...
                  <Row>
                    <Col>
                      ...
                      >
                        ...
                        <Form.Control
                          type="text"
                          value={thesisDetail?.studentName || thesisDetail?.student?.fullName || 'N/A'}
                          readOnly
                        />
                      </Form.Group>
                    </Col>
                    <Col>
                      ...
                      >
                        ...
                        <Form.Control
                          type="text"
                          value={thesisDetail?.facultyName || thesisDetail?.faculty?.name || 'N/A'}
                          readOnly
                        />
                      </Form.Group>
                    </Col>
            ...
            >
              <Modal.Header closeButton>
                <Modal.Title>
                  ...
                  {!loadingExistingMark && markingData.score.format !== '' && markingData.score.format !== -1 && (
              ...
              <Modal.Body>
                ...
                {!loadingExistingMark && markingData.score.format !== '' && markingData.score.format !== -1 && (
                  <div className="alert alert-info mb-3">
                    ...
                    Khóa luận này đã được chấm điểm. Bạn có thể chỉnh sửa các điểm dưới đây.
                  </div>
                )}
                <Form>
                  <Row>
                    <Col md={6}>
                      <Form.Group className="mb-3">
                        <Form.Label>Định dạng (Format) <span className="text-danger">*</span></Form.Label>
                        <Form.Control
                          type="number"
                          min="0"
                          max="10"
                          ...
                          onChange={(e) => handleScoreChange('format', e.target.value)}
                          placeholder="Nhập điểm"
                          isInvalid={markingData.score.format !== '' && (isNaN(parseFloat(markingData.score.format)) || parseFloat(markingData.score.format) < 0 || parseFloat(markingData.score.format) > 10)}
                        />
                    ...
                    <Col md={6}>
                      <Form.Group className="mb-3">
                        <Form.Label>Nội dung (Content) <span className="text-danger">*</span></Form.Label>
                        <Form.Control
                          type="number"
                          min="0"
                          max="10"
                          ...
                          placeholder="Nhập điểm"
                          isInvalid={markingData.score.content !== '' && (isNaN(parseFloat(markingData.score.content)) || parseFloat(markingData.score.content) < 0 || parseFloat(markingData.score.content) > 10)}
                        />
                        <Form.Control.Feedback type="invalid">
                          Điểm phải là số từ 0 đến 10
                        </Form.Control.Feedback>
                  ...
                  <Row>
                    <Col md={6}>
                      <Form.Group className="mb-3">
                        <Form.Label>Nghiên cứu (Research) <span className="text-danger">*</span></Form.Label>
                        <Form.Control
                          type="number"
                          min="0"
                          max="10"
                          ...
                          placeholder="Nhập điểm"
                          isInvalid={markingData.score.research !== '' && (isNaN(parseFloat(markingData.score.research)) || parseFloat(markingData.score.research) < 0 || parseFloat(markingData.score.research) > 10)}
                        />
                        <Form.Control.Feedback type="invalid">
                          Điểm phải là số từ 0 đến 10
                        </Form.Control.Feedback>
                    ...
                    <Col md={6}>
                      <Form.Group className="mb-3">
                        <Form.Label>Thuyết trình (Presentation) <span className="text-danger">*</span></Form.Label>
                        <Form.Control
                          type="number"
                          min="0"
                          max="10"
                          ...
                          placeholder="Nhập điểm"
                          isInvalid={markingData.score.presentation !== '' && (isNaN(parseFloat(markingData.score.presentation)) || parseFloat(markingData.score.presentation) < 0 || parseFloat(markingData.score.presentation) > 10)}
                        />
                        <Form.Control.Feedback type="invalid">
                          Điểm phải là số từ 0 đến 10
                        </Form.Control.Feedback>
