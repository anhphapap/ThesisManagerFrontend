...
const Council = () => {
  ...
  const loadLecturerOption = async (facultyId) => {
    try {
      ...
      const validLecturers = Array.isArray(res.data)
        ? res.data.filter((l) => l && l.id)
        : [];
  ...
  const handleCloseCouncil = async (councilId) => {
    if (
      window.confirm(
        "Bạn có chắc chắn muốn khóa hội đồng này? Hành động này sẽ hoàn thành tất cả khóa luận, gửi email thông báo đến sinh viên và không thể hoàn tác."
      )
    ) {
      try {
        ...
        alert(
          "Khóa hội đồng thành công! Email thông báo đã được gửi đến sinh viên."
        );
                ...
                {loading ? (
                  ...
                  councils.map((council, index) => (
                    ...
                    >
                      ...
                      <td>{council.name}</td>
                      <td>
                        {council.faculty?.name || council.facultyName || "N/A"}
                      </td>
                      <td>
                        <span
                          className={`badge ${
                            council.status ? "bg-success" : "bg-secondary"
                          }`}
                        >
          ...
          {councilDetail && (
            <Form>
                ...
                <Col md={6}>
                  <Form.Group>
                    ...
                    <Form.Control
                      ...
                      className={
                        councilDetail.status ? "text-success" : "text-secondary"
                      }
                      readOnly
                    />
                  </Form.Group>
              ...
              <Table bordered>
                ...
                <tbody>
                  {(councilDetail.lecturers || councilDetail.members) &&
                  (councilDetail.lecturers || councilDetail.members).length >
                    0 ? (
                    (councilDetail.lecturers || councilDetail.members).map(
                      (member, index) => (
                      ...
                      <td>
                        <Form.Group>
                          <Form.Select
                            ...
                            onChange={(e) =>
                              updateMember(index, "lecturerId", e.target.value)
                            }
                          >
                            <option value="">Chọn giảng viên</option>
                            {Array.isArray(lecturerOption) &&
                              lecturerOption
                                .filter((l) => l && l.id)
                                .map((l) => (
